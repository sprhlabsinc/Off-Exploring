...
{ 
	...
	if([elementName isEqualToString:@"result"])
	{
		...
		if(currentAddressComponent != nil)
		{
			...
			for (NSString *string in currentAddressComponent.types) {
				if ([string isEqualToString:@"locality"]) {
					currentResult.localityName = currentAddressComponent.longName;
				}
				else if ([string isEqualToString:@"administrative_area_level_1"]) {
					currentResult.subAdministrativeAreaName = currentAddressComponent.longName;
				}
				else if ([string isEqualToString:@"country"]) {
					currentResult.countryName = currentAddressComponent.longName;
					currentResult.countryNameCode = currentAddressComponent.shortName;
				}
			}
